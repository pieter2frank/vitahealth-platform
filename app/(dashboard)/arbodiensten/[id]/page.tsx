...
export default async function ArbodienstDetailPage({
  ...
  const { data: arbo } = await supabase.from('vh_arbo').select('*').eq('id', id).single()
  if (!arbo) notFound()

  const { data: testkits } = await supabase
    .from('vh_testkit')
    .select('id, barcode, date, status, retour_date')
    .eq('assigned_arbo_id', id)
    .order('date', { ascending: false })

  if (bewerken === '1') return <EditArboForm arbo={arbo} />
    ...
    <div className="p-8 max-w-7xl">
      <div className="mb-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-[#1e293b]">{arbo.name}</h1>
            <p className="text-sm text-[#64748b] mt-0.5">Aangemaakt op {formatDate(arbo.created_at)}</p>
          </div>
          <div className="flex items-center gap-2">
            <Link
              href={`/arbodiensten/${id}?bewerken=1`}
            ...
            <DeleteButton
              table="vh_arbo"
              id={arbo.id}
              redirectTo="/arbodiensten"
              entityLabel={arbo.name}
              blockedMessage={
                testkits && testkits.length > 0
                  ? `Er ${testkits.length === 1 ? 'is' : 'zijn'} ${testkits.length} testkit${testkits.length === 1 ? '' : 's'} gekoppeld aan deze arbodienst. Ontkoppel of verwijder de testkits eerst voordat je de arbodienst verwijdert.`
                  : undefined
              }
            />
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-[#e2e8f0] bg-white p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-[#1e293b] mb-4 flex items-center gap-2">
            <Stethoscope size={15} className="text-[#94a3b8]" />
            Gegevens
          </h2>
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between">
              <dt className="text-[#64748b]">Naam</dt>
              <dd className="font-medium text-[#1e293b]">{arbo.name}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-[#64748b]">Contactpersoon</dt>
              <dd className="font-medium text-[#1e293b]">{arbo.contact_name ?? '—'}</dd>
            </div>
          </dl>
        </div>
        ...
        <div className="rounded-xl border border-[#e2e8f0] bg-white p-5 shadow-sm">
          ...
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between">
              ...
              <dd className="font-medium text-[#1e293b]">{arbo.email ?? '—'}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-[#64748b] flex items-center gap-1.5"><Phone size={13} />Telefoon</dt>
              <dd className="font-medium text-[#1e293b]">{arbo.phone ?? '—'}</dd>
            </div>
            <div className="flex justify-between">
              ...
              <dd className="font-medium text-[#1e293b] text-right">
                {arbo.address ? <>{arbo.address}<br />{arbo.postal_code} {arbo.city}</> : '—'}
              </dd>
            </div>
          </dl>
        </div>
