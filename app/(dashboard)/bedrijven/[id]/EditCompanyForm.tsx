...
export function EditCompanyForm({ company }: { company: Company }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.push(`/bedrijven/${company.id}`)
    router.refresh()
  }
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="mb-6">
        <Link href={`/bedrijven/${company.id}`} className="inline-flex items-center gap-1.5 text-sm text-[#64748b] hover:text-[#1e293b] mb-4">
          <ArrowLeft size={14} />
          Terug
        </Link>
        <h1 className="text-2xl font-bold text-[#1e293b]">{company.name} bewerken</h1>
      </div>
      <form onSubmit={handleSubmit} className="rounded-xl border border-[#e2e8f0] bg-white p-6 shadow-sm space-y-6">
        <div>
          ...
          <div className="space-y-4">
            <Input label="Bedrijfsnaam" value={form.name} onChange={e => set('name', e.target.value)} required />
            <Input label="KVK-nummer" value={form.kvk} onChange={e => set('kvk', e.target.value)} />
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button type="button" variant="outline" onClick={() => router.push(`/bedrijven/${company.id}`)}>Annuleren</Button>
        </div>
      </form>
    </div>
  )
