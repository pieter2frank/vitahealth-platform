...
export default async function BedrijvenPage({
  ...
  let query = supabase
    .from('vh_company')
    .select('id, name, contact_name, email, phone, city, kvk, created_at')
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#1e293b]">Bedrijven</h1>
          <p className="text-sm text-[#64748b] mt-0.5">
            {companies?.length ?? 0} bedrijf/bedrijven gevonden.
          </p>
        </div>
        <Link
      ...
      <div className="rounded-xl border border-[#e2e8f0] bg-white shadow-sm overflow-hidden">
        {error ? (
          <div className="px-5 py-8 text-center text-sm text-red-500">Fout bij ophalen bedrijven.</div>
        ) : !companies || companies.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <p className="text-sm text-[#94a3b8] mb-3">
              {q ? `Geen bedrijven gevonden voor "${q}".` : 'Nog geen bedrijven aangemaakt.'}
            </p>
            <Link href="/bedrijven/nieuw" className="inline-flex items-center gap-1.5 text-sm text-[#1f1683] hover:underline">
              <Building2 size={14} />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#e2e8f0] bg-[#f8fafc]">
                ...
                <th className="px-4 py-3 text-left font-medium text-[#64748b]">KVK</th>
            ...
            <tbody className="divide-y divide-[#f1f5f9]">
              {companies.map((c) => (
                <ClickableRow key={c.id} href={`/bedrijven/${c.id}`} className="hover:bg-[#f8fafc] transition-colors">
                  <td className="px-4 py-3 font-medium text-[#1e293b]">{c.name}</td>
                  <td className="px-4 py-3 text-[#64748b]">{c.contact_name ?? '—'}</td>
                  <td className="px-4 py-3 text-[#64748b]">{c.email ?? '—'}</td>
                  <td className="px-4 py-3 text-[#64748b]">{c.city ?? '—'}</td>
                  <td className="px-4 py-3 text-[#64748b]">{c.kvk ?? '—'}</td>
