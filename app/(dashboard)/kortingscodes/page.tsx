...
export default async function KortingscodesPage() {
  ...
  const [{ data: codes }, { data: packages }, { data: resellers }] = await Promise.all([
    admin.from('vh_discount_code')
      .select('id, code, type, value, package_id, reseller_id, max_uses, used_count, valid_until, active, note, created_at')
      .order('created_at', { ascending: false }),
    admin.from('vh_package').select('id, name').order('sort_order', { ascending: true }),
    admin.from('vh_reseller').select('id, name'),
  ])
  ...
  const resellerName = new Map((resellers ?? []).map(r => [r.id as string, r.name as string]))
  ...
  const rows: CodeRow[] = (codes ?? []).map(c => ({
    ...
    resellerName: c.reseller_id ? (resellerName.get(c.reseller_id as string) ?? null) : null,
