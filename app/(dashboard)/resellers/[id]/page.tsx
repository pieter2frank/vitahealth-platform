...
import { ResellerDetail, type ResellerData, type ResellerCode, type PackageOption } from './ResellerDetail'
...
export default async function ResellerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: reseller }, { data: codes }, { data: packages }] = await Promise.all([
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <ResellerDetail data={data} codes={codeRows} packages={pkgOptions} />
    </div>
  )
}
