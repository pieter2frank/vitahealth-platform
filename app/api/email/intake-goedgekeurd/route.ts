...
export async function POST(req: Request) {
  ...
  const statusUrl = tokenRow?.token
    ...
    : `${portalUrl}/portal/aanmelden`

  const { subject, html } = intakeGoedgekeurdEmail({
  ...
  if (error) {
    console.error('[email/intake-goedgekeurd] Resend error:', error)
  ...
  logAuditEvent({
    ...
    action:          'email_sent',
    outcome:         'success',
    reason:          'Intake-goedkeuring verstuurd',
  }).catch(() => {})
