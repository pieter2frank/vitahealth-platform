...
| **Wijzigingen v1.2** | Fasen 4 en 5 afgerond (rotatie, retentie/anonimiseren, order-kopergegevens versleuteld, legacy-opschoning). |
| **Status** | Alle fasen (0–5) afgerond (13 augustus 2026) |
| **Eigenaar** | Vita Health |
| **Rollback-anker** | git-tag `pre-pii-kluis` (commit `80923c3`) |

> Markdown-versie van `docs/pii-kluis-implementatieplan-v1.2.docx`. Bij een inhoudelijke
> wijziging: nieuw bestand met opgehoogd versienummer; oude versies blijven als historie.

---

## 0. Uitvoeringsstatus (v1.2)
...
| 4 — Rotatieprocedure (geversioneerde sleutels + rotatie-run), anonimiseer-actie, audit bij bulk-ontsleuteling | ✅ Afgerond |
| 5 — Kopergegevens op bestellingen versleuteld (AAD per order, legacy-passthrough + backfill); legacy aanvraag-pagina verwijderd; notitie-beleid in de UI | ✅ Afgerond |
