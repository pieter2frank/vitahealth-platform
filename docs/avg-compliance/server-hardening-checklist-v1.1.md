...
| **Status** | Vastgesteld — eerste volledige uitvoering 18 augustus 2026 (beide servers) |
| **Wijzigingen v1.1** | Cloud-init-valkuil bij SSH-hardening toegevoegd (geleerde les eerste uitvoering). |
| **Eigenaar** | Vita Health |

> Markdown-versie van `docs/server-hardening-checklist-v1.1.docx`. Context: op deze servers
...
⚠️ **Valkuil (geleerd 18 aug 2026)**: controleer de *effectieve* instelling altijd met
`sudo sshd -T | grep -i passwordauthentication` — een configbestand met "no" zegt niets.
Bij OpenSSH wint de **eerst gelezen** waarde, en cloud-init zet bij het aanmaken van de VPS
vaak `/etc/ssh/sshd_config.d/50-cloud-init.conf` neer met `PasswordAuthentication yes`
(alleen leesbaar met sudo — een grep zonder sudo mist hem!). Zo stond wachtwoord-login op
de app-server effectief áán terwijl 60-cloudimg "no" zei. Oplossing: een eigen bestand dat
vóór de 50 sorteert:
...
En vergeet niet: **eerst** de eigen publieke sleutel in `authorized_keys` zetten en
sleutel-login testen in een tweede terminal — op de app-server moet daarnaast de
coolify-sleutel blijven staan (deploys!).
