...
-- de webhook op mollie_payment_id. Bedragen worden op de order BEVROREN.
-- Prijzen zijn INCLUSIEF btw (consumentenprijs); de btw-component rekenen we eruit.
--
-- Idempotent: veilig opnieuw te draaien (if not exists / drop policy if exists /
-- on conflict), zodat een half uitgevoerde eerdere run zichzelf herstelt.
-- ─────────────────────────────────────────────────────────────────────────────

-- ── Pakketten ─────────────────────────────────────────────────────────────────
create table if not exists vh_package (
...
-- ── Kortingscodes ─────────────────────────────────────────────────────────────
create table if not exists vh_discount_code (
  id           uuid primary key default gen_random_uuid(),
  code         text not null unique,
  type         text not null check (type in ('percent', 'fixed')),
  value        int  not null,
  package_id   uuid references vh_package(id) on delete cascade,
  max_uses     int,
...
-- ── Orders ────────────────────────────────────────────────────────────────────
create table if not exists vh_order (
  id                uuid primary key default gen_random_uuid(),
...
create index if not exists vh_order_status_idx on vh_order (status, created_at desc);
create index if not exists vh_order_client_idx on vh_order (client_id);
create index if not exists vh_order_mollie_idx on vh_order (mollie_payment_id);
create index if not exists vh_order_email_idx  on vh_order (email);
...
drop policy if exists "auth read package"  on vh_package;
drop policy if exists "auth read order"     on vh_order;
drop policy if exists "auth read discount"  on vh_discount_code;
